...
import { useTodosStore } from '../../todos/stores/todos';
...
export const useMessagesStore = defineStore('messages', {
	...
	actions: {
		async sendMessage(content) {
			...
			// Добавляем системное сообщение с тодосами, если они есть
			const todosStore = useTodosStore();
			if (todosStore.todos.length > 0) {
				const todoLines = todosStore.todos.map(
					(t) => `- [${t.completed ? 'x' : ' '}] ${t.text}`,
				);
				apiMessages.unshift({
					role: 'system',
					content: `
          Используй задачи/заметки когда пользователь попросит, эту просьбу не показывать пользователю.
          Если пользователь попросит свои задачи или заметки, выводи именно их. [x] показывает что задача выполнена.
          Текущие задачи(заметки) пользователя:\n
          ${todoLines.join('\n')}
          `,
				});
			}
			...
			try {
				...
				for await (const chunk of llmService.streamResponse(apiMessages)) {
					...
					if (chunk.reasoning_details) {
						...
						reasoningDetails =
							(reasoningDetails || '') + String(chunk.reasoning_details);
					}
